...
if st.session_state.page == "questions":
    ...
    st.subheader("① エネルギーの向き（E/I）")
    q1 = st.slider("一人より大勢でいるほうが元気になる", 1, 5, 3, key="q1",
                   help="1=一人が好き ／ 5=大勢が好き")
    q2 = st.slider("初対面の人ともすぐに打ち解けられる", 1, 5, 3, key="q2",
                   help="1=時間がかかる ／ 5=すぐ打ち解ける")
    st.divider()

    st.subheader("② 情報の受け取り方（S/N）")
    q3 = st.slider("新しいやり方より、実績ある手順を好む", 1, 5, 3, key="q3",
                   help="1=新しいやり方派 ／ 5=実績ある手順派")
    q4 = st.slider("将来の可能性より、今の現実を重視する", 1, 5, 3, key="q4",
                   help="1=可能性重視 ／ 5=現実重視")
    st.divider()

    st.subheader("③ 判断の基準（T/F）")
    q5 = st.slider("感情より論理・データで決断する", 1, 5, 3, key="q5",
                   help="1=感情・人間関係派 ／ 5=論理・データ派")
    q6 = st.slider("相手が傷ついても、率直な批評をする", 1, 5, 3, key="q6",
                   help="1=気持ちを優先する ／ 5=率直に言う")
    st.divider()

    st.subheader("④ 生活スタイル（J/P）")
    q7 = st.slider("事前に計画を立てないと落ち着かない", 1, 5, 3, key="q7",
